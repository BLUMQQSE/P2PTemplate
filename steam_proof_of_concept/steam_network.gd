...
func _create_host():
	...
	if error == OK:
		multiplayer_peer.transfer_mode = MultiplayerPeer.TRANSFER_MODE_RELIABLE
		get_parent().get_node("Network").call("SetNetworkState", 1)
...
func connect_socket(steam_id: int):
	...
	multiplayer_peer.transfer_mode = MultiplayerPeer.TRANSFER_MODE_RELIABLE
	get_parent().get_node("Network").call("SetNetworkState", 2)
